...
public class SafeCacheTest {
    ...
    void acquireAndRelease_ShouldReuseSameInstance() throws Exception {
        ...
        SafeCache<Integer, TestResource> cache =
                new SafeCache<>(Runtime.getRuntime().availableProcessors() * 2, _ -> new TestResource(loaderCount.incrementAndGet()));
    ...
    void overLimit_ShouldEvictSomeEntries() throws Exception {
        ...
        SafeCache<Integer, TestResource> cache =
                new SafeCache<>(Runtime.getRuntime().availableProcessors() * 2, key -> new TestResource(counter.incrementAndGet()));

        // Fill cache
        TestResource r1 = cache.acquire(0);
        cache.release(0);
        for (int i = 1; i <= 33; i++) {
            TestResource r = cache.acquire(i);
            cache.release(i);
        }
    ...
    void close_ShouldCloseAndClearCache() throws Exception {
        SafeCache<String, TestResource> cache =
                new SafeCache<>(Runtime.getRuntime().availableProcessors() * 2, key -> new TestResource(1));
